...
Geocodes an `Address` column via the public [Nominatim](https://nominatim.org/) API (`nominatim.openstreetmap.org/search`, `format=jsonv2&extratags=1`), writing back `Latitude`, `Longitude`, `OsmType`, `OsmId`, `AdminLevel`, and `AddressRank`. No map/Leaflet dependency — it's a plain data-enrichment widget with a small status panel (current address, output fields, activity log).
