...
- **app.js** — all behavior, in a few small blocks:
  ...
  - **Grist sync**: `grist.ready({ requiredAccess: "full" })` initializes the widget. `grist.onRecord()` fires when the selected record changes — it only overwrites the editor's contents if the editor isn't currently focused (`document.activeElement !== editor`), so the widget doesn't clobber in-progress typing. Column mapping is read from the `markdown` key in the `mappings` argument, defaulting to a column literally named `markdown` if unmapped. On every editor `input` event, the widget both re-renders locally and pushes the new value back to Grist via `grist.selectedTable.update()`, keyed on `currentRecordId`.
