...
5. [Choropleth Map](/choropleth-map) : Renders a GeoJson column (one feature, or a FeatureCollection, per row) as a choropleth using [Observable Plot](https://observablehq.com/plot/)'s geo mark, coloring each feature by a Value column with a choice of continuous/quantile/equal-interval color scales; projection, scheme, and legend/graticule/sphere options are all configurable from a toolbar in the widget itself
   - Widget URL to use in Grist: `https://vcityteam.github.io/grist-widgets/choropleth-map`
